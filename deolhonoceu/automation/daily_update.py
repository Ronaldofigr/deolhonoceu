...
# ── FOTO DA SEMANA (NASA APOD) ───────────────────────────────────────────────

def update_photo_week():
    """Troca a foto da semana toda segunda-feira usando NASA APOD."""
    import datetime, json, urllib.request
    from pathlib import Path

    today = datetime.date.today()
    # Só atualiza às segundas-feiras
    if today.weekday() != 0:
        print("  ⏭  Foto da semana: não é segunda-feira, mantendo atual.")
        return

    week = today.strftime("%Y-W%W")
    foto_path = Path(__file__).parent.parent / "content" / "foto-semana.json"

    # Verifica se já atualizou esta semana
    if foto_path.exists():
        current = json.loads(foto_path.read_text())
        if current.get("week") == week:
            print("  ⏭  Foto da semana já atualizada.")
            return

    print(f"  🌠 Buscando nova foto da semana (NASA APOD)...")

    try:
        # Busca APOD da NASA (sem chave, limite de 30/hora)
        url = "https://api.nasa.gov/planetary/apod?api_key=DEMO_KEY&count=7"
        with urllib.request.urlopen(url, timeout=10) as resp:
            items = json.loads(resp.read())

        # Filtra apenas fotos (não vídeos)
        fotos = [i for i in items if i.get("media_type") == "image"]
        if not fotos:
            print("  ⚠️  Nenhuma foto disponível no APOD esta semana.")
            return
        ...
        credit = apod.get("copyright", "NASA / APOD").strip() if apod.get("copyright") else "NASA / APOD"

        # Usa Claude para traduzir e adaptar a legenda
        prompt = f"""Você recebeu esta descrição de uma imagem astronômica da NASA:

TÍTULO: {title_en}
DESCRIÇÃO: {explanation[:800]}

Crie uma legenda em português acessível para leigos (máx 120 palavras) e traduza o título.
Responda SOMENTE com JSON:
{{"title": "título em português", "caption": "legenda em português acessível", "captionEn": "caption in English max 120 words"}}"""
        ...
        foto_data = {
            ...
            "titleEn": title_en,
            "caption": data.get("caption", explanation[:300]),
            "captionEn": data.get("captionEn", explanation[:300]),
            "credit": credit,
            "week": week
        }

        foto_path.write_text(json.dumps(foto_data, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"  ✅  Nova foto da semana: {data.get('title', title_en)}")

    except Exception as e:
        print(f"  ⚠️  Erro ao buscar foto da semana: {e}")
